    ...
    def self.call
        ...
        schedulers.each do |scheduler|
            ActiveRecord::Base.transaction do
                date  = DateTime.now
                log_entry = SchedulerLog.where(tenant: scheduler.tenant, donuts_scheduler: scheduler, scheduler_success: true).where('created_at >= ? and created_at <=?', date.midnight, date.end_of_day)
