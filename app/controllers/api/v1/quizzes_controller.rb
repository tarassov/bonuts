class Api::V1::QuizzesController < Api::V1::ApiController
  ...
  def index
    quizes = []
    if @current_tenant
      quizes = Donut.where(tenant_id: @current_tenant.id, active: quiz_params[:active],
                           closed: quiz_params[:closed])
    end
  ...
  def create
    ...
    if response.status != :ok
      ...
      json_response(QuizSerializer.new(response.result, {}).serializable_hash.to_json, :ok, response.result,
                    :bad_request)
    end
  end
