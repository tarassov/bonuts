...
describe Transfer do
  before(:context) do
    ...
    profiles_to_select_from = @tenant.profiles.select do |p|
      p.id != @profileFrom.id && p.department && p.department.head_profile.id != p.id
    end
  ...
  context 'when not enought dounts' do
    before do
      result = DepositAction.call({ account: @profileFrom.distrib_account,
                                    amount: @intial_balance - @profileFrom.distrib_account.balance })
