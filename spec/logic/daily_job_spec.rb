...
describe DailyJob do
  before(:context) do
    ...
    DonutsScheduler.create({ day: DateTime.now.day, active: true, every: 'daily', tenant: @tenant,
                             profile: @profileAdmin, amount: 10, burn_old: false })
    DonutsScheduler.create({ day: DateTime.now.day + 1, active: true, every: 'daily', tenant: @tenant,
                             profile: @profileAdmin, amount: 10, burn_old: false })
    @d = DailyJob.call
    @d2 = DailyJob.call
  end
